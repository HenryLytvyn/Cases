...
// Создаем разметку карточек и добавляем её в HTML
...
// Добавляем слушателя используя делегирование событий
container.addEventListener("click", handleAdd);

function handleAdd(event) {
  // Отлавливаем карточку по клике на кнопку
  ...
  // Вытягиваем значение id с этой карточки
  const productId = Number(parent.dataset.id);
  console.log(typeof productId);

  // Находим по этому id обьект в исходном массиве товара
  const currentProduct = instruments.find(({ id }) => id === productId);
  console.log(currentProduct);

  // Создаем переменную в которую записываем распарсяное значение из ключа в localStorage, где будут хранится данные о продуктах в корзине или пустой массив
  const products = JSON.parse(localStorage.getItem(LS_KEY)) ?? []; // JSON.parse() роспарсивает значение из localStorage создавая массив со строки
  console.log(products);

  // Находим индекс нашего товара по id в этом массиве с помощью .findIndex
  const index = products.findIndex(({ id }) => id === productId);

  // Если нашего товара там нету мы копируем обьект товара с оригинального массива в новую переменую и добавляем этому обьекту свойство, которое обозначает количество = 1 и добавляем его в массив наших продуктов хранящихся в корзине на localStorage. Если наш товар там уже есть тогда +1 к количеству.
  if (index === -1) {
  ...
  // Добавляем (или перезаписываем) в localStorage наш обьект с обновленным свойством количества в виде строки.
